...
class TestEMGMeasurementCollection(TestCase):
    def setUp(self):
        ...
        self.c = EMGMeasurementCollection(all_data, hz)

    def tearDown(self):
        pass

    def test_apply_dc_offset_remover(self):
        self.c.apply_dc_offset_remover()
        actual = self.c.all_data
        desired = [
    ...
    def test_apply_full_wave_rectifier(self):
        self.c.apply_full_wave_rectifier()
        actual = self.c.all_data
        desired = [
            np.array([20.3, 41.0, 53.9, 63.3, 39.5, 24.9, 26.1, 24.0, 44.1, 42.0,
                      37.4, 24.6, 21.8, 56.3, 48.1, 45.0, 29.1, 9.6, 5.3, 1.4]),
    ...
    def test_apply_linear_envelope(self):
        ...
        self.c.apply_linear_envelope(le_order=le_order, le_cutoff_fq=le_cutoff_fq)
        actual = self.c.all_data
        desired = [
    ...
    def test_apply_end_frame_cutter(self):
        ...
        self.c.apply_end_frame_cutter(n_end_frames=n_end_frames)
        actual = self.c.all_data
        desired = [
    ...
    def test_find_max_amplitude_of_each_channel_across_trials(self):
        ...
        actual = self.c.find_max_amplitude_of_each_channel_across_trials()
        np.testing.assert_allclose(actual, desired)

    def test_apply_amplitude_normalizer(self):
        max_amplitude = 10

        self.c.apply_amplitude_normalizer(max_amplitude=max_amplitude)
        actual = self.c.all_data
        desired = [
    ...
    def test_apply_segmenter(self):
        ...
        self.c.apply_segmenter(all_beg_ts=all_beg_ts, all_end_ts=all_end_ts)
        actual = self.c.all_data
        desired = [
